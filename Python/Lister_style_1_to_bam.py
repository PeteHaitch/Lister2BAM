...
### Program description ###
############################################################################################################################################################################################
# Convert the H1_r1, H1_r2, IMR90_r1 or IMR90_r2 MethylC-Seq mapped reads files from the Lister et al. 2009 (Nature) paper (downloaded from http://neomorph.salk.edu/human_methylome/data.html on 08/05/2012) to BAM format.
############################################################################################################################################################################################
...
def makeQNAME(RNAME, ID):
    qname = '_'.join([RNAME, ID])
    return qname
    
def makePOS(start, sequence):
    start = int(start) # 0-based leftmost mapping position
    return start
        
def makeFLAG(sequence, strand):
    flag = 0x0
    if strand == '-':
        flag += 0x10
    return flag
...
for line in INFILE: # Loop over the file line-by-line and convert to an AlignedRead instance
    ...
    QNAME = makeQNAME(RNAME, ID)
    FLAG = makeFLAG(sequence, strand)
    POS = makePOS(start, sequence)
    MAPQ = makeMAPQ()
    CIGAR = makeCIGAR(sequence)
    RNEXT = makeRNEXT(RNAME) 
    PNEXT= makePNEXT(POS)
    TLEN = makeTLEN(start, end)
